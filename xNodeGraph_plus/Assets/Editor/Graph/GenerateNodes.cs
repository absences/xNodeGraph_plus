...
namespace GEditor.Graph {
    public static class GenerateNodes {
        private const string CODE = @"using Cysharp.Threading.Tasks;
using Game.Graph;
...
{CallAsync}
        ...
        private const string AWAIT_CODE = "await UniTask.CompletedTask;";
        ...
        public static Dictionary<Type, Type> TypeMapping = new Dictionary<Type, Type>() {
            ...
            {typeof(System.Boolean), typeof(Boolen)},
        ...
        private static bool IsValueType(Type type, bool convertObj=false) {
            ...
            if (gtypes != null && gtypes.Length > 0 && type.BaseType == typeof(UniTask).BaseType) {
                type = gtypes[0];
            }
        ...
        private static string ConvertType(Type type, bool convertObj=false) {
            ...
            if (gtypes != null && gtypes.Length > 0) {
                if (type.BaseType == typeof(UniTask).BaseType) {
                    type = gtypes[0];
                }
                else {
        ...
        private static bool IsVoidType(Type type) {
            ...
            var gtypes = type.GenericTypeArguments;
            return type == typeof(UniTask) && (gtypes == null || gtypes.Length == 0);
        }
    }
}